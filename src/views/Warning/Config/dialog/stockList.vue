    ...
    >
      <el-form :model="data.form" ref="addSensorForm" :rules="formRules">
        <el-card class="sbsub" v-if="!data.form.sensorid">
          <el-form-item label="">
            <SensorPicker :sensorData.sync="data.sensorData"></SensorPicker>
            <!-- {{ data.sensorData }} -->
          </el-form-item>
          <el-row>
            <el-button
...
export default {
  ...
  setup(props, { root, emit, refs }) {
    ...
    onMounted(() => {
      ...
      /* watch */
      //       watch(
      //   () => data.dialog_stock_flag,
      //   (newValue, oldValue) => (data.dialog_stock_flag = props.flag)
      // );
      watch(
        () => props.flag,
    ...
    const close = () => {
      ...
      // console.log(data.sbTypeSelect.init);
    ...
    // 获取传感器详情
    const getsensorMsg = () => {
      reqSensorsMsg(data.sensorData.sensorsValue).then(res => {
        console.log(res);
